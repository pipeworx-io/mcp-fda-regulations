...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/fda_regulation \
  -H 'Content-Type: application/json' \
  -d '{"citation":"211.100"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/fda_regulation`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "fda-regulations": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-fda-regulations"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-fda-regulations
```
